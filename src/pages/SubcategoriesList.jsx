...
import { useSubcategoriesQuery } from '../features/subcategories/useSubcategoriesQuery';
import { useMemo, useEffect, useState } from 'react';
import { getAdminCategories } from '../services/admin';
import { useQueryClient } from '@tanstack/react-query';
import { deleteSubcategory } from '../services/admin';
...
function EmptyState({ onAdd }) {
  return (
    <div className="text-center py-16">
      ...
      <div className="mt-4">
        <h3 className="text-lg font-semibold text-gray-900">No subcategories found</h3>
        <p className="text-gray-500 mt-1">Get started by creating a new subcategory.</p>
      </div>
      <button
        onClick={onAdd}
        className="mt-6 inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-primary to-secondary text-white font-medium rounded-xl hover:from-secondary hover:to-cyan-700 shadow-lg shadow-primary/30 transition-all duration-200"
      >
        <PlusIcon />
        Add Subcategory
      </button>
...
function SubcategoriesList() {
  ...
  const params = useMemo(
    ...
    [page, limit, q, active, categoryId]
  );
  const { data, isLoading } = useSubcategoriesQuery(params);
  ...
  const onDelete = async () => {
    ...
    await deleteSubcategory(deleteModal.item.id);
  ...
  const items = useMemo(() => {
    ...
    return itemsBase.filter((s) => {
      const matchesText =
        !v ||
        (s.name || '').toLowerCase().includes(v) ||
        (s.slug || '').toLowerCase().includes(v) ||
        (s.category_name || '').toLowerCase().includes(v);
      const matchesActive = active === '' || Number(s.is_active) === Number(active);
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {items.map((s, index) => (
                      <tr 
                        key={s.id} 
                        className="group hover:bg-gradient-to-r hover:from-teal-50/50 hover:to-cyan-50/50 transition-all duration-200"
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          {s.category_name ? (
                            <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium bg-primary/10 text-primary shadow-sm`}>
                        ...
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold ${
                            ...
                            <span className={`w-1.5 h-1.5 rounded-full ${s.is_active ? 'bg-emerald-500 animate-pulse' : 'bg-gray-400'}`} />
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center justify-end gap-2">
                            <button 
                              className="p-2.5 rounded-xl bg-gray-100 text-gray-600 hover:bg-primary/20 hover:text-secondary hover:shadow-lg hover:shadow-primary/20 transform hover:scale-105 transition-all duration-200" 
              ...
              <div className="px-6 py-4 bg-gradient-to-r from-gray-50 to-gray-100/50 border-t border-gray-100">
                <div className="flex items-center justify-between">
                  <div className="text-sm text-gray-500">
                    Showing <span className="font-semibold text-gray-700">{items.length}</span> subcategories
                    {totalPages > 1 && (
